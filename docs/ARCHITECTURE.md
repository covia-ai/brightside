...
│                               mode (onboard / unlock / legacy), takeover, chat
...
├── vault/Vault.java            passphrase → Argon2id → vault key (Etch v3) + seed key
...
└── ui/
    ...
    ├── WelcomePanel.java       "What should I call you?" (legacy first run, rename)
    ├── SettingsDialog.java     File → Model & API key…
